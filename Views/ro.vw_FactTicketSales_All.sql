...
( SELECT f.OrderDate TransDateTime
		, f.ETL__SSID_TM_acct_id, f.ETL__SourceSystem
		...
		, f.DimRepId
		, f.TM_ticket_seq_id
  FROM [ro].[vw_FactTicketSales] (NOLOCK) f

UNION ALL

  SELECT fts.OrderDate TransDateTime
		, fts.ETL__SSID_TM_acct_id, fts.ETL__SourceSystem
  ...
  FROM [ro].[vw_FactTicketSales_History] fts (NOLOCK)
