...
/***************
7/14/2017 - AMEITIN added additional piece to matchkey logic so we use the field that we are populating with Paciolan ID's 
...
--base set
		...
		'Oregon_SFDC' AS [SourceDB]
		, 'Oregon PC_SFDC Account' AS [SourceSystem]
		...
		/*AddressPrimary*/
			...
			, LEFT(Business_Other_Phone__c,25) AS [PhoneBusiness]
			...
			, a.[Patron_ID__c] AS [ExtAttribute5] 
			...
			, CASE WHEN (SELECT value FROM [dbo].[fn_Split](a.[Patron_ID__c],', ') WHERE position = 1) IS NOT NULL
				THEN 'TM-' + (SELECT value FROM [dbo].[fn_Split](a.[Patron_ID__c],', ') WHERE position = 1)
				END AS customer_matchkey

--			select top 10 * 
			FROM Oregon_Reporting.prodcopy.account a (NOLOCK)
			WHERE a.isdeleted = 0
			AND a.CopyLoadDate > DATEADD(DAY,-3,GETDATE())
